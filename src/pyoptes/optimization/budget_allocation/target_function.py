...
def n_infected_animals(is_infected):
    return np.sum(capacities * is_infected) #is_infected is bool
...
def evaluate(budget_allocation, 
    ...
    for i in range(len(results)):
        shares_detected.append(results[i][0])
        per_node.append(results[i][1])
        total_infections.append(results[i][2])
    # return the requested statistic:
    
    return statistic(np.array(shares_detected), np.array(per_node), np.array(total_infections))
